...
// Check if user is logged in and has permission to edit tickets
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['user_role'], ['admin', 'department_admin', 'staff'])) {
...
// Get staff members for assignment (based on user role)
$staff_members = [];
try {
    if ($user_role == 'admin') {
        ...
        // Department admin and staff can only assign within their department
        $stmt = $pdo->prepare("
            SELECT id, CONCAT(first_name, ' ', last_name) as name, email, role
            FROM users 
...
// Get categories for the ticket's department
$categories = [];
try {
    $stmt = $pdo->prepare("
        SELECT id, name 
        ...
        ORDER BY name
    ");
    $stmt->execute([$ticket['department_id']]);
